...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            #Emmascarem els valors de padding perquè no afectin a l'atenció
            attn_scores = attn_scores.masked_fill(mask, -1e9)
            
        # XAI: Obtenim les probabilitats (pesos d'atenció real)
        attn_probs = F.softmax(attn_scores, dim=1)
        
        # Fem la suma ponderada
        weighted_sum = torch.sum(x * attn_probs.unsqueeze(-1), dim=1)
        
        return weighted_sum, attn_probs
...
class LogTransformer(nn.Module):
    def __init__(self, vocab_size, embedding_dim=128, num_heads=4, num_layers=3, dropout=0.2):
        ...
        # Clasificador Dual (Attention + Mean Pooling)
    ...
    def forward(self, x, return_attention=False):
        ...
        transformer_out = self.transformer_encoder(x_pos, src_key_padding_mask=src_key_padding_mask)
        
        # 1. Extracció de característiques per Atenció
        attn_out, attn_probs = self.attention_pooling(transformer_out, mask=src_key_padding_mask)
        
        # 2. Extracció de característiques globals per Mean Pooling 
        transformer_out_masked = transformer_out.masked_fill(src_key_padding_mask.unsqueeze(-1), 0.0)
